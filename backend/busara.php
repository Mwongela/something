...
if(isset($_POST['application']) && isset($_POST['user'])) {
	...
	if($respodentResultSet) {
		...
		if(mysql_num_rows($respodentResultSet) > 0) {
			...
			// insert
			$respodentInsert = mysql_query("INSERT INTO `respondents` (`id`, `survey_id`, `application`) VALUES (NULL, '$surveyId', '$application')");
		...
		if(isset($_POST['analytics'])) {
			...
			foreach ($data as $key => $value) {
				...
				$pageAnalyticsQuery = "INSERT INTO `page_statistics` (`id`, `respondent_id`, `timestamp`, `timespent`, `previousPage`, `pageName`, `pageOrder`, `isInputPresent`) VALUES (NULL, '$respondentId', '$timeStamp', '$timeSpent', '$previousPage', '$pageName', '$pageOrder', '$isInputPresent')";
				...
				if($pageQueryResultSet) {
					...
					if($isInputPresent == "yes") {
						...
						$inputName = $inputStats['name'];

						$inputAnalyticsQuery = "INSERT INTO `input_stats` (`id`, `name`, `page_statistic_id`, `backspaceCount`, `totalKeyPressCount`, `timeStartTyping`, `timeStopTyping`, `timeSpentInField`, `finalInputValue`, `finalInputLength`, `intelliWordChanges`, `intelliWordIndex`) VALUES (NULL, '$inputName', '$pageStatsId', '$backspaceCount', '$totalKeyPressCount', '$timeStartTyping', '$timeStopTyping', '$timeSpentInField', '$finalInputValue', '$finalInputLength', '$intelliWordChanges', '$intelliWordIndex')";
